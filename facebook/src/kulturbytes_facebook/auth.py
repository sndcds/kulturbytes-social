...
def request(client: httpx.Client, url: str, token: str, **params: str) -> dict:
    try:
        ...
        raise click.ClickException(
            "Facebook: Netzwerkfehler bei der Authentifizierung."
        ) from None
    try:
    ...
    if failed or error or not isinstance(payload, dict):
        ...
        message = (
            f"Facebook: Zugriff fehlgeschlagen. HTTP {response.status_code} [REDACTED]"
        )
        if type(code) is int:
            message += f" (Code {code})"
        if code == 190:
            message += (
                ...
                if subcode == 463
                else " Token ist ungültig oder abgelaufen."
            )
...
def validate(client: httpx.Client, base: str, page_id: str, token: str) -> str:
    ...
    if payload.get("id") != page_id:
        raise click.ClickException(
            "Facebook: Zurückgegebene Seiten-ID stimmt nicht mit FACEBOOK_PAGE_ID überein."
        )
    ...
    for _ in range(1000):
        ...
        for page in pages:
            if (
                ...
                or not page["id"].isascii()
                or not page["id"].isdigit()
                or not isinstance(page.get("name"), str)
                or not page["name"].strip()
            ):
                raise click.ClickException(
                    "Facebook: Ungültiger Eintrag in der Seitenliste."
                )
            if page["id"] == page_id:
                token = page.get("access_token")
                if not isinstance(token, str) or not token.strip():
                    raise click.ClickException(
                        "Facebook: Für die konfigurierte Seite fehlt der Page Access Token."
                    )
        ...
        if next_url is not None:
            ...
            try:
                ...
                safe = (
                    ...
                    and parsed.path == f"/{graph_api_version}/me/accounts"
            ...
            if not safe or not isinstance(after, str) or not after or after in seen:
                raise click.ClickException(
                    "Facebook: Ungültige oder zyklische Pagination."
                )
            seen.add(after)
        ...
        if len(matches) > 1:
            raise click.ClickException(
                "Facebook: Konfigurierte Seite ist nicht eindeutig."
            )
        if matches:
            return matches[0]
        if next_url is None:
            raise click.ClickException(
                "Facebook: FACEBOOK_PAGE_ID wurde in /me/accounts nicht gefunden."
            )
    raise click.ClickException("Facebook: Zu viele Seiten der Pagination.")


def authenticate_legacy_page(
    page_id: str, version: str, *, force: bool = False, secrets: list[str] | None = None
) -> str:
    ...
    with httpx.Client(
        ...
        follow_redirects=False,
        headers={
            "User-Agent": "Kulturbytes-Facebook-Publisher/1.0",
        ...
        if page_token:
            try:
                name = validate(
                    client, f"https://graph.facebook.com/{version}", page_id, page_token
                )
                ...
                click.echo(
                    redact(f"✓ Seite erreichbar: {name} ({page_id})", page_token)
                )
                return page_token
        tty = interactive()
        if (
            ...
            and not click.confirm(
                "Mit einem User Access Token wiederherstellen?", default=False
            )
        ):
        ...
        if not user_token and tty:
            user_token = click.prompt(
                "Facebook User Access Token", hide_input=True
            ).strip()
        if not user_token:
            ...
            raise click.ClickException(
                (
                    ...
                    if original_error
                    else "Facebook Page Access Token fehlt."
                )
                + suffix
            )
        ...
        if tty and click.confirm(
            "Page Access Token im OS-Keyring speichern?", default=True
        ):
...
def authenticate_page(
    ...
    with httpx.Client(
        ...
        token = resolve_page_access_token(
            client,
            user_access_token=system_token,
            page_id=page_id,
            graph_api_version=version,
        )
        secrets.append(token)
