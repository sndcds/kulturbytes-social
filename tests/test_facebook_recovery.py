...
class RecoveryTests(IsolatedEnvironmentTestCase):
    def invoke(
        self, responses, *, env=None, stored=None, tty=False, input="", args=None
    ):
        calls = []
        ...
        with (
            ...
            patch(
                "kulturbytes_common.credentials.get_secret", side_effect=get_secret
            ) as get,
            ...
            result = CliRunner().invoke(
                cli, ["facebook", *(args or ["--check-auth"])], input=input
            )
    ...
    def test_valid_page_precedence(self):
        for env, stored in [
            ({"FACEBOOK_PAGE_ACCESS_TOKEN": OLD}, {"page-access-token": NEW}),
            ({}, {"page-access-token": OLD}),
        ]:
            result, calls, save, get = self.invoke(
                [(200, PAGE)], env=env, stored=stored
            )
            self.assertEqual(result.exit_code, 0, result.output)
            self.assertEqual(calls[0].headers["Authorization"], f"Bearer {OLD}")
            self.assertNotIn(
                "user-access-token", [c.args[1] for c in get.call_args_list]
            )
    ...
    def test_expired_env_and_keyring_recovery(self):
        for env, stored in [
            (
                {"FACEBOOK_PAGE_ACCESS_TOKEN": OLD, "FACEBOOK_USER_ACCESS_TOKEN": USER},
                {"user-access-token": "ignored"},
            ),
            ({}, {"page-access-token": OLD, "user-access-token": USER}),
        ]:
            result, calls, save, _ = self.invoke(
                [(400, EXPIRED), (200, ACCOUNTS), (200, PAGE)], env=env, stored=stored
            )
            self.assertEqual(result.exit_code, 0, result.output)
            self.assertEqual(
                [r.headers["Authorization"] for r in calls],
    ...
    def test_pagination_and_exact_match(self):
        first = {
            "data": [{"id": "999", "name": "Kulturbytes", "access_token": "wrong"}],
            "paging": {
                "next": "https://graph.facebook.com/v26.0/me/accounts?access_token=unsafe",
                "cursors": {"after": "cursor-two"},
            },
        }
        result, calls, _, _ = self.invoke(
            [(200, first), (200, ACCOUNTS), (200, PAGE)],
    ...
    def test_malformed_missing_and_unsafe_accounts(self):
        bodies = [
            ...
            {
                "data": [],
                "paging": {"next": "https://graph.facebook.com/v26.0/me/accounts"},
            },
        ...
        for body in bodies:
            with self.subTest(body=body):
                result, calls, save, _ = self.invoke(
                    [(200, body)], env={"FACEBOOK_USER_ACCESS_TOKEN": USER}
                )
    ...
    def test_invalid_validation_and_permissions_fail_closed(self):
        for body in [
            ...
            result, _, save, _ = self.invoke(
                [(200, ACCOUNTS), (200, body)], env={"FACEBOOK_USER_ACCESS_TOKEN": USER}
            )
            self.assertNotEqual(result.exit_code, 0)
            save.assert_not_called()
        for code in (10, 200):
            result, calls, _, _ = self.invoke(
                [(403, {"error": {"code": code}})],
                env={
                    "FACEBOOK_PAGE_ACCESS_TOKEN": OLD,
    ...
    def test_hidden_prompt_and_optional_save(self):
        for answer in ("y", "n"):
            with patch.object(auth.click, "prompt", wraps=auth.click.prompt) as prompt:
                result, _, save, _ = self.invoke(
                    [(400, EXPIRED), (200, ACCOUNTS), (200, PAGE)],
                    tty=True,
            ...
            if answer == "y":
                save.assert_called_once_with(
                    "kulturbytes-social/facebook", "page-access-token", NEW
                )
            else:
                save.assert_not_called()
    ...
    def test_explicit_resolution_and_failed_publish_are_read_only(self):
        ...
        result, _, save, _ = self.invoke(
            [(400, EXPIRED), (403, {"error": {"code": 200}})],
    ...
    def test_cycle_and_empty_env_override(self):
        body = {
            "data": [],
            "paging": {
                "next": "https://graph.facebook.com/v26.0/me/accounts",
                "cursors": {"after": "same"},
            },
        }
        result, calls, _, _ = self.invoke(
            [(200, body)] * 2, env={"FACEBOOK_USER_ACCESS_TOKEN": USER}
        )
        self.assertNotEqual(result.exit_code, 0)
        self.assertEqual(len(calls), 2)
        result, calls, _, get = self.invoke(
            [],
            env={"FACEBOOK_PAGE_ACCESS_TOKEN": "", "FACEBOOK_USER_ACCESS_TOKEN": ""},
            stored={"page-access-token": OLD, "user-access-token": USER},
        )
        self.assertNotEqual(result.exit_code, 0)
    ...
    def test_publish_uses_recovered_token_after_preflight(self):
        ...
        self.assertEqual(
            [r.url.path for r in calls[:3]],
            ["/v26.0/123", "/v26.0/me/accounts", "/v26.0/123"],
        )
    ...
    def test_transport_and_keyring_failure_do_not_expose_secrets(self):
        with (
            ...
            patch(
                ...
                side_effect=KeyringUnavailable("OS-Keyring ist nicht verfügbar."),
            ),
            patch.object(auth, "interactive", return_value=False),
        ):
    ...
    def test_management_cannot_swallow_resolve_action(self):
        result, calls, _, _ = self.invoke(
            [], args=["--credentials", "status", "--resolve-page-token"]
        )
