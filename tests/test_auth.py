from publisher_harness import cli as auth_cli
from kulturbytes_common.errors import RemoteRejected
import os
import subprocess
import sys
import tempfile
...
ENV = {'FACEBOOK_PAGE_ID': '123', 'FACEBOOK_PAGE_ACCESS_TOKEN': TOKEN, 'MASTODON_BASE_URL': 'https://norden.social', 'MASTODON_ACCESS_TOKEN': TOKEN, 'INSTAGRAM_USER_ID': '123', 'INSTAGRAM_ACCESS_TOKEN': TOKEN}
...
class AuthTests(IsolatedEnvironmentTestCase):
    ...
    def test_import_and_help_without_credentials(self):
        for module in PLATFORMS:
            with self.subTest(platform=module.__name__), patch.dict(os.environ, {}, clear=True):
                result = subprocess.run([sys.executable, '-c', f'import {module.__name__}'], capture_output=True, text=True, check=False)
    ...
    def test_publication_requests_use_lazy_credentials(self):
        cases = [(FACEBOOK, FACEBOOK.publish_text_post, '/v26.0/123/feed', 'post-123'), (FACEBOOK, FACEBOOK.publish_facebook_photo, '/v26.0/123/photos', 'post-123'), (MASTODON, MASTODON.upload_mastodon_media, '/api/v2/media', 'post-123'), (MASTODON, MASTODON.publish_mastodon_status, '/api/v1/statuses', ('post-123', None))]
        for module, publish, path, expected in cases:
            for status in [200, 401]:
                with self.subTest(function=publish.__name__, status=status), patch.dict(os.environ, ENV, clear=True):
                    ...
                    with httpx.Client(transport=httpx.MockTransport(respond)) as client, patch.object(module, 'download_image', return_value=(b'image', 'image/jpeg', 'image.jpg')):
                        if status == 200:
                            self.assertEqual(publish(client, EVENT), expected)
                        else:
                            with self.assertRaises((click.ClickException, RemoteRejected)) as error:
    ...
    def run_auth(self, module, response, *, env=None, args=None, expected_exit=0):
        ...
        with patch.dict(os.environ, ENV if env is None else env, clear=True), patch('kulturbytes_common.auth.httpx.Client', side_effect=make_client):
            result = CliRunner().invoke(auth_cli, [module.__name__.split('.')[0].removeprefix('kulturbytes_')] + (args or ['--check-auth']))
        ...
        self.assertEqual(len(requests), 3 if isinstance(response, (httpx.ConnectError, httpx.ReadTimeout)) or (isinstance(response, httpx.Response) and response.status_code in (429, 502, 503, 504)) else 1)
        ...
        for module in PLATFORMS:
            for login in ['instagram', 'facebook']:
                with self.subTest(platform=module.__name__, login=login):
                    ...
                    result, request = self.run_auth(module, httpx.Response(200, json=payload), env=dict(ENV, INSTAGRAM_LOGIN_TYPE=login), args=['--check-auth', '--publish', '--city', 'Hamburg', '--limit', '0', '--include-published', '--event-uuid', 'ignored'])
    ...
    def test_auth_failures_and_redaction(self):
        for module in PLATFORMS:
            for status in [400, 401, 403, 500]:
                for payload in [{'error': {'code': 190, 'message': TOKEN}}, {'error': {'code': 190, 'error_subcode': 463, 'message': quote(TOKEN, safe='')}}, {'error': TOKEN}]:
    ...
    def test_invalid_account_responses(self):
        for module in PLATFORMS:
            ...
            if module is not MASTODON:
                result, _ = self.run_auth(module, httpx.Response(200, json={'id': '999', 'name': 'Wrong page', 'username': 'wrong-account'}), expected_exit=1)
    ...
    def test_success_output_redacts_echoed_token(self):
        for module in PLATFORMS:
            with self.subTest(platform=module.__name__):
                result, _ = self.run_auth(module, httpx.Response(200, json={'id': '123', 'name': TOKEN, 'username': TOKEN, 'acct': TOKEN}))
                self.assertIn('[REDACTED]', result.output)
    ...
    def test_missing_credentials_fail_before_network_or_database(self):
        for module in PLATFORMS:
            for option in ['--publish', '--check-auth']:
                with self.subTest(platform=module.__name__, option=option), patch.dict(os.environ, {}, clear=True), patch('httpx.Client') as client:
                    result = CliRunner().invoke(auth_cli, [module.__name__.split('.')[0].removeprefix('kulturbytes_'), option])
