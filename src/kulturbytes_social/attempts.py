...
@click.option(
    "--state", type=click.Choice(STATES), default=None, help="Nach Zustand filtern."
)
@click.option(
    "--active", is_flag=True, help="Nur reserved, publishing und remote_succeeded."
)
...
@click.option(
    ...
    default=50,
    show_default=True,
    help="Neueste N Versuche; 0 zeigt alle.",
)
def show_attempts(
    platform: str, state: str | None, active: bool, date_uuid: str | None, limit: int
) -> None:
    ...
    try:
        rows = list_attempts(
            conn, state=state, active=active, date_uuid=date_uuid, limit=limit
        )
        for row in rows:
...
def resolve(
    ...
    try:
        ...
        if not click.confirm(
            "Publisher gestoppt, Plattform manuell geprüft und dieses Ergebnis sicher bestätigt?",
            default=False,
        ):
