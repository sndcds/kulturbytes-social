...
class MediaSecurityTests(IsolatedEnvironmentTestCase):
    ...
    def test_dns_mixed_private_and_recheck_each_retry(self):
        ...
        for ips in [("10.0.0.1",), ("8.8.8.8", "127.0.0.1")]:
            with (
                patch("socket.getaddrinfo", return_value=records(*ips)),
        ...
        with (
            ...
            httpx.Client(
                transport=httpx.MockTransport(
                    lambda r: (calls.append(r), httpx.Response(503))[1]
                )
            ) as client,
        ):
            with self.assertRaises(click.ClickException):
    ...
    def test_redirect_validation_and_limits(self):
        for location in (
            ...
            with httpx.Client(
                ...
                with self.assertRaises(click.ClickException):
                    with media_response(
                        client, "https://api.kulturbytes.de/start", POLICY
                    ):
                        pass
        calls = []
        ...
        def safe(request):
            ...
            return (
                httpx.Response(302, headers={"Location": "/finish"})
        ...
        with httpx.Client(
            transport=httpx.MockTransport(
                lambda r: httpx.Response(302, headers={"Location": str(r.url) + "x"})
            )
        ) as client:
            with self.assertRaises(click.ClickException):
    ...
    def test_rebinding_cannot_change_tcp_destination_and_proxies_are_ignored(self):
        ...
        def connect_tcp(backend, host, port, **kwargs):
            ...
            test.assertTrue(
                actual.is_global, "Rebinding reached a private TCP endpoint"
            )
            return Stream()
        ...
        with (
            ...
            patch(
                ...
                side_effect=REAL_MEDIA_CLIENT,
            ),
            httpx.Client(
                trust_env=False, headers={"Authorization": "Bearer test-secret"}
            ) as parent,
        ):
            with media_response(
                parent, "https://api.kulturbytes.de/api/image/example", POLICY
            ) as response:
    ...
    def test_allowlist_rejects_external_hosts_ports_and_credential_variants(self):
        for url in (
            ...
            "https://api.kulturbytes.de.evil.test/image",
            "https://api.kulturbytes.de:444/image",
            "https://user:pass@api.kulturbytes.de/image",
