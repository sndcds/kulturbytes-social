...
EVENT = {
    ...
    "date": {
        ...
        "venue_street": "Hauptstraße",
        "venue_house_number": "1",
...
class SharedFunctionsTests(IsolatedEnvironmentTestCase):
    ...
    def test_formatting(self):
        ...
        self.assertEqual(
            build_hashtags(EVENT), "#Kultur #SchleswigHolstein #Kulturbytes #Flensburg"
        )
    ...
    def test_image_download(self):
        ...
        event["images"] = {
            "main": {"url": "https://api.kulturbytes.de/image", "uuid": "image-1"}
        }
        with httpx.Client(
            ...
            self.assertEqual(
                download_image(client, event), (b"image", "image/png", "image-1.png")
            )
...
class PublisherTests(IsolatedEnvironmentTestCase):
    def run_cli(
        self, module, directory, args, user_input, summaries=None, expected_exit=0
    ):
        requests = []
        ...
        def respond(request):
            ...
            if request.url.path == "/api/events":
                return httpx.Response(
                    ...
                    json={
                        "data": {
                            "events": summaries if summaries is not None else [SUMMARY]
                        }
                    },
                )
        ...
        with (
            ...
            patch.object(MASTODON, "get_status_limit", return_value=500),
            patch.dict(os.environ, ENV, clear=True),
            patch.object(module, "DATABASE_PATH", Path(directory) / "posts.sqlite3"),
            patch("kulturbytes_common.workflow.httpx.Client", return_value=client),
        ):
            result = CliRunner().invoke(
                cli,
                [module.__name__.split(".")[0].removeprefix("kulturbytes_")] + args,
                input=user_input,
            )
        self.assertEqual(
            result.exit_code, expected_exit, result.output + str(result.exception)
        )
    ...
    def test_dry_run_for_both_platforms(self):
        for module in [FACEBOOK, MASTODON]:
            with (
                self.subTest(platform=module.__name__),
    ...
    def test_existing_database_and_duplicate_filter(self):
        for module in [FACEBOOK, MASTODON]:
            with (
                ...
                with patch.object(
                    module, "DATABASE_PATH", Path(directory) / "posts.sqlite3"
                ):
                    conn = module.init_database()
                    if module is FACEBOOK:
                        module.remember_post(conn, EVENT, "post-1")
                    else:
                        module.remember_post(
                            conn, EVENT, "post-1", "https://example.test/post-1"
                        )
                ...
                result, requests = self.run_cli(
                    module, directory, ["--include-published"], "1\ny\n"
                )
            ...
            try:
                if module is FACEBOOK:
                    ...
                    module.remember_post(
                        ...
                        previous_event,
                        "old-post",
                        "https://example.test/old-post",
                    )
                # Avoid sleeps and same-second timestamp comparisons.
                conn.execute(
                    "UPDATE published_events SET published_at = '2000-01-01 00:00:00'"
                )
                conn.commit()
            finally:
                conn.close()
    ...
    def test_confirmed_repeat_publish_updates_existing_record(self):
        for module in [FACEBOOK, MASTODON]:
            for status_url in ["https://example.test/new-post", None]:
                with (
                    self.subTest(platform=module.__name__, status_url=status_url),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    ...
                    function = (
                        "publish_text_post"
                        if module is FACEBOOK
                        else "publish_mastodon_status"
                    )
                    value = (
                        "new-post" if module is FACEBOOK else ("new-post", status_url)
                    )
                    ...
                    with patch.object(
                        module, function, side_effect=remote_success
                    ) as publish:
                        result, _ = self.run_cli(
                            module,
                            directory,
                            ["--include-published", "--publish"],
                            "1\ny\ny\n",
                        )
                    ...
                    self.assertGreater(
                        record["published_at"], previous[0]["published_at"]
                    )
                    id_column = (
                        "facebook_post_id"
                        if module is FACEBOOK
                        else "mastodon_status_id"
                    )
    ...
    def test_failed_repeat_publish_preserves_existing_record(self):
        for module in [FACEBOOK, MASTODON]:
            with (
                ...
                with patch.object(
                    module, function, side_effect=RuntimeError("Remote failure")
                ) as publish:
                    result, _ = self.run_cli(
                        module,
                        directory,
    ...
    def test_repeat_publish_requires_both_confirmations_and_respects_dry_run(self):
        ...
        for module in [FACEBOOK, MASTODON]:
            for args, user_input in cases:
                with (
                    self.subTest(
                        platform=module.__name__, args=args, user_input=user_input
                    ),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    previous = self.seed_previous_publication(module, directory)
        ...
        for module in [FACEBOOK, MASTODON]:
            for identifier in ["209901011830", "date-1"]:
                with (
                    ...
                    value = (
                        "post-direct" if module is FACEBOOK else ("post-direct", None)
                    )
    ...
    def test_direct_selection_rejects_invalid_or_filtered_targets(self):
        ...
        for module in [FACEBOOK, MASTODON]:
            for summaries in [
                [],
                [dict(SUMMARY, uuid="other")],
                [dict(SUMMARY, date_uuid="other")],
                [SUMMARY, SUMMARY],
                [dict(SUMMARY, release_status="draft")],
                [dict(SUMMARY, start_date="2000-01-01")],
            ]:
                with (
                    self.subTest(platform=module.__name__, summaries=summaries),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    _, requests = self.run_cli(
                        module, directory, args, "", summaries, expected_exit=1
                    )
                    self.assertEqual(len(requests), 1)
            for incomplete in [
                ["--event-uuid", "event-1"],
                ["--date-identifier", "date-1"],
            ]:
                with (
                    self.subTest(platform=module.__name__, args=incomplete),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    result, requests = self.run_cli(
                        module, directory, incomplete, "", expected_exit=2
                    )
        ...
        for module in [FACEBOOK, MASTODON]:
            with (
                ...
                value = (
                    "post-2"
                    if module is FACEBOOK
                    else ("post-2", "https://example.test/post-2")
                )
                with patch.object(module, function, return_value=value) as publish:
