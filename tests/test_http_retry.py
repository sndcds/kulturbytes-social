...
class RetryTests(IsolatedEnvironmentTestCase):
    ...
    def test_inherits_client_timeout_and_preserves_explicit_overrides(self):
        ...
        for streaming in (False, True):
            for kwargs, expected in (({}, configured.as_dict()), ({'timeout': override}, override.as_dict()),
                                     ({'timeout': None}, httpx.Timeout(None).as_dict())):
                seen = []
                def respond(request):
                    seen.append(request.extensions['timeout'])
                    return httpx.Response(503 if len(seen) == 1 else 200)
                with httpx.Client(timeout=configured, transport=httpx.MockTransport(respond)) as client:
                    response = safe_get(client, 'https://example.test', stream=streaming, **kwargs)
                    response.close()
