...
def open_database(path: Path, platform: str) -> sqlite3.Connection:
    ...
    try:
        ...
        columns = {row[1] for row in conn.execute('PRAGMA table_info(published_events)')}
        if columns and (PLATFORM_COLUMNS[platform] not in columns or
                        any(column in columns for other, column in PLATFORM_COLUMNS.items() if other != platform)):
            raise click.ClickException('Datenbankschema gehört zu einer anderen Plattform; separaten Datenbankpfad konfigurieren.')
        conn.execute('CREATE TABLE IF NOT EXISTS publisher_metadata (singleton INTEGER PRIMARY KEY CHECK(singleton=1), platform TEXT NOT NULL)')
        owner = conn.execute('SELECT platform FROM publisher_metadata WHERE singleton=1').fetchone()
        if owner and owner[0] != platform:
            raise click.ClickException('Datenbank ist bereits einer anderen Plattform zugeordnet; separaten Pfad konfigurieren.')
        conn.execute('INSERT OR IGNORE INTO publisher_metadata VALUES (1, ?)', (platform,))
