...
def validate_detail(payload: object) -> dict:
    try:
        ...
        return EventDetail.model_validate(payload["data"]).model_dump(
            exclude_unset=True
        )
    except (ValidationError, ValueError):
        raise click.ClickException(
            "Kulturbytes: ungültige Detaildaten oder date_uuid/date-Struktur; Veröffentlichung abgebrochen."
        ) from None


def validate_list(
    payload: object, *, target: tuple[str, str] | None = None
) -> list[dict]:
    if (
        not isinstance(payload, dict)
        or not isinstance(payload.get("data"), dict)
        or not isinstance(payload["data"].get("events"), list)
    ):
        raise click.ClickException(
            "Kulturbytes: ungültige Listenantwort; data.events muss eine Liste sein."
        )
    valid = []
    for index, item in enumerate(payload["data"]["events"], 1):
        try:
            valid.append(
                EventSummary.model_validate(item).model_dump(exclude_unset=True)
            )
        except ValidationError:
            if (
                target
                and isinstance(item, dict)
                and item.get("uuid") == target[0]
                and target[1] in (item.get("date_uuid"), item.get("date_slug"))
            ):
                raise click.ClickException(
                    "Kulturbytes: der direkt gewählte Termin enthält ungültige Listendaten."
                ) from None
            click.echo(
                f"Kulturbytes: ungültiger Listeneintrag {index} übersprungen.", err=True
            )
    return valid
