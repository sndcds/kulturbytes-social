...
_current: ContextVar[tuple[sqlite3.Connection, str] | None] = ContextVar(
    "publication_attempt", default=None
)
...
def transaction(conn: sqlite3.Connection) -> Iterator[None]:
    ...
    if conn.in_transaction:
        raise click.ClickException(
            "Unerwartete laufende Datenbanktransaktion; Vorgang abgebrochen."
        )
    try:
...
def validate_target_ref(platform: str, value: str | None) -> str | None:
    ...
    if platform in ("facebook", "instagram"):
        if not re.fullmatch(r"[0-9]{1,100}", value):
            raise click.ClickException(
                "Zielkennung muss eine numerische Konto-ID sein."
            )
        return value
    try:
        ...
        raise click.ClickException(
            "Journal-Ziel muss eine Instanzadresse ohne Zugangsdaten sein."
        ) from None
...
def init_journal(conn: sqlite3.Connection) -> None:
    ...
    columns = {
        row[1] for row in conn.execute("PRAGMA table_info(publication_attempts)")
    }
...
def list_attempts(
    ...
    *,
    ...
    if active and state and state not in ACTIVE:
        raise click.ClickException(
            "--active kann nicht mit einem abgeschlossenen --state kombiniert werden."
        )
    if limit < 0:
...
def get_attempt(conn: sqlite3.Connection, attempt_uuid: str) -> dict:
    cursor = conn.execute(
        "SELECT * FROM publication_attempts WHERE attempt_uuid=?", (attempt_uuid,)
    )
    row = cursor.fetchone()
    if row is None:
...
def unresolved_attempt(
    conn: sqlite3.Connection, date_uuid: str, platform: str | None = None
) -> dict | None:
    cursor = conn.execute(
        """SELECT * FROM publication_attempts
        WHERE date_uuid=? AND (? IS NULL OR platform=?)
        AND state IN ('reserved','publishing','remote_succeeded') LIMIT 1""",
        (date_uuid, platform, platform),
    )
    row = cursor.fetchone()
...
def describe_attempt(attempt: dict) -> str:
    return (
        f"Plattform={attempt['platform']}, date_uuid={attempt['date_uuid']}, "
...
def reserve_attempt(
    ...
    *,
    ...
    snapshot["date"] = {
        key: event["date"].get(key)
        for key in ("uuid", "slug", "start_date", "start_time")
    }
    with transaction(conn):
        ...
        if (
            not allow_repeat
            and conn.execute(
                "SELECT 1 FROM published_events WHERE date_uuid=?", (date_uuid,)
            ).fetchone()
        ):
            raise click.ClickException(
                "Termin wurde inzwischen veröffentlicht; keine erneute Veröffentlichung."
            )
        conn.execute(
...
def _transition(
    ...
    *,
    ...
    cursor = conn.execute(
        f"""UPDATE publication_attempts SET state=?,
    ...
    if cursor.rowcount != 1:
        raise click.ClickException(
            "Veröffentlichungszustand wurde geändert; Vorgang abgebrochen."
        )
    ...
    if current:
        ...
        with transaction(conn):
            if not stage.startswith(get_attempt(conn, attempt_uuid)["platform"] + "_"):
                raise click.ClickException(
                    "Veröffentlichungsphase passt nicht zur Plattform."
                )
            _transition(
                conn,
                attempt_uuid,
...
def mark_remote_succeeded(
    ...
    if not isinstance(remote_id, str) or not remote_id:
        raise click.ClickException(
            "Remote-Erfolg ohne gültige ID; manuelle Prüfung erforderlich."
        )
    with transaction(conn):
        _transition(
            conn,
...
def execute_publication(
    ...
    *,
    ...
    try:
        ...
        attempt = get_attempt(conn, attempt_uuid)
        raise click.ClickException(
            "Remote-Ergebnis ungeklärt; erneutes Posten gesperrt. "
            + describe_attempt(attempt)
            + ". Mit kulturbytes-social attempts prüfen."
        ) from None
    finally:
...
def resolve_attempt(
    ...
    attempt_uuid: str,
    outcome: str,
    finalize: Callable[[dict, str, str | None], None],
    *,
    remote_id: str | None = None,
    remote_url: str | None = None,
) -> None:
    ...
    with transaction(conn):
        ...
        if attempt["remote_id"]:
            if remote_id and remote_id != attempt["remote_id"]:
                raise click.ClickException(
                    "Remote-ID darf einen bestätigten Erfolg nicht überschreiben."
                )
            if remote_url and remote_url != attempt["remote_url"]:
                raise click.ClickException(
                    "Remote-URL darf einen bestätigten Erfolg nicht überschreiben."
                )
            ...
            if remote_url:
                ...
                if not safe_remote_url(remote_url, ""):
                    raise click.ClickException(
                        "Remote-URL darf keine Zugangsdaten oder Query-Parameter enthalten."
                    )
