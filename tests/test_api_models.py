...
class APIModelTests(IsolatedEnvironmentTestCase):
    def test_envelope_and_sibling_resilience(self):
        ...
        for key in ("uuid", "date_uuid"):
            ...
            self.assertEqual(
                validate_list({"data": {"events": [bad, SUMMARY]}}), [SUMMARY]
            )
        self.assertIsNone(
            validate_list({"data": {"events": [{**SUMMARY, "venue_city": None}]}})[0][
                "venue_city"
            ]
        )
    ...
    def test_direct_invalid_match_cannot_be_hidden_by_valid_sibling(self):
        ...
        with self.assertRaises(click.ClickException):
            validate_list(
                {"data": {"events": [SUMMARY, bad]}},
                target=(SUMMARY["uuid"], SUMMARY["date_uuid"]),
            )
    ...
    def test_detail_fields_and_opaque_ids(self):
        ...
        for update in (
            ...
            {"images": []},
            {"images": {"main": []}},
            {"images": {"main": {"url": "file:///etc/passwd"}}},
            {"date": {}},
            {"date": None},
        ):
