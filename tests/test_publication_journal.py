...
class JournalTests(IsolatedEnvironmentTestCase):
    ...
    def test_success_repeat_and_legacy_records(self):
        ...
        execute_publication(
            self.conn, "facebook", EVENT, self.publish, self.finalize, allow_repeat=True
        )
    ...
    def test_remote_success_survives_actual_sqlite_finalization_failure_and_recovery(
        self,
    ):
        self.conn.execute(
            "CREATE TRIGGER reject_finalization BEFORE INSERT ON published_events BEGIN SELECT RAISE(ABORT, 'disk failure'); END"
        )
        self.conn.commit()
        with self.assertRaises(click.ClickException) as error:
            execute_publication(
                self.conn, "facebook", EVENT, self.publish, self.finalize
            )
        ...
        resolve_attempt(
            ...
            "published",
            lambda event, post_id, url: FACEBOOK.remember_post(
                self.conn, event, post_id, commit=False
            ),
        )
        self.assertTrue(already_published(self.conn, "date-1"))
        self.assertEqual(
            get_attempt(self.conn, attempt["attempt_uuid"])["state"], "published"
        )

    def test_failure_to_journal_remote_id_retains_committed_blocker(self):
        with patch(
            "kulturbytes_common.publications.mark_remote_succeeded",
    ...
    def test_recovery_cli_confirms_and_never_makes_http_requests(self):
        ...
        with patch.object(
            httpx.Client, "send", side_effect=AssertionError("unexpected HTTP")
        ):
            listed = CliRunner().invoke(
                cli, ["attempts", "list", "--platform", "facebook"]
            )
    ...
    def test_unresolved_blocks_direct_even_with_include_published(self):
        ...
        client = httpx.Client(
            transport=httpx.MockTransport(
                lambda r: httpx.Response(200, json={"data": {"events": [SUMMARY]}})
            )
        )
    ...
    def test_all_platforms_journal_after_confirmation_and_before_remote(self):
        helpers = {
            ...
            MASTODON: "publish_mastodon_status",
            INSTAGRAM: "publish_instagram_photo",
        }
        for module, helper in helpers.items():
            ...
            with (
                ...
                patch.object(
                    module,
                    "DATABASE_PATH",
                    Path(self.directory.name) / f"{platform}-integration.sqlite3",
                ),
            ):
                    ...
                    begin_remote_mutation(
                        {
                            ...
                            "mastodon": "mastodon_status",
                            "instagram": "instagram_publish",
                        }[platform]
                    )
                    return (
                        ("123", "https://example.test/123")
                        if module is MASTODON
                        else "123"
                    )

                with (
                    patch.object(module, helper, side_effect=remote) as call,
                    patch("click.confirm", return_value=False),
                    patch.object(
                        INSTAGRAM,
                        "validate_image",
                        return_value="https://api.kulturbytes.de/image.jpg",
                    ),
                ):
    ...
    def test_real_platform_post_paths_are_reserved_and_never_retried(self):
        ...
        for module, config in configs.items():
            for result_kind in ("success", "403", "503", "timeout", "invalid_id"):
                ...
                with (
                    self.subTest(platform=platform, result=result_kind),
                    ...
                    def respond(request):
                        if request.method == "POST":
                            self.assertEqual(
                                list_attempts(conn)[0]["state"], "publishing"
                            )
                            ...
                            if result_kind in ("403", "503"):
                                return httpx.Response(
                                    int(result_kind),
                                    json={"error": {"message": "test-secret rejected"}},
                                )
                            return httpx.Response(
                                200,
                                json={
                                    "id": "test-secret"
                                    if result_kind == "invalid_id"
                                    else "123",
                    ...
                    with (
                        ...
                        if result_kind == "success":
                            module.publish_event(
                                client, conn, canonical(event), False, config=config
                            )
                        else:
                            with self.assertRaises(click.ClickException) as error:
                                module.publish_event(
                    ...
                    self.assertEqual(
                        len(posts),
                        2 if module is INSTAGRAM and result_kind == "success" else 1,
                    )
    ...
    def test_remote_url_cannot_leak_credentials(self):
        ...
        self.assertEqual(
            remote_url("https://example.test/@account/123", "test-secret"),
    ...
    def test_content_fingerprint_is_deterministic_and_context_is_minimal(self):
        ...
        self.assertEqual(
            fingerprint, content_fingerprint("FACEBOOK", deepcopy(EVENT), text)
        )
        ...
        attempt = reserve_attempt(
            self.conn, "facebook", EVENT, message=text, target_ref="123"
        )
    ...
    def test_additive_context_migration_preserves_old_attempts(self):
        ...
        with sqlite3.connect(":memory:") as legacy:
            ...
            legacy.execute(
                "INSERT INTO publication_attempts VALUES ('old','facebook','date-1','event-1', 'remote_succeeded','123',NULL,NULL,'{}','2000','2000')"
            )
            init_journal(legacy)
            init_journal(legacy)
            row = get_attempt(legacy, "old")
            self.assertEqual(
                (row["state"], row["remote_id"]), ("remote_succeeded", "123")
            )
            for field in (
                "mutation_stage",
                "date_slug",
    ...
    def test_recovery_is_one_atomic_transaction_and_rolls_back_all_local_writes(self):
        ...
        self.conn.execute(
            "CREATE TRIGGER reject_published BEFORE UPDATE ON publication_attempts WHEN NEW.state='published' BEGIN SELECT RAISE(ABORT, 'final transition failed'); END"
        )
        self.conn.commit()
        attempt = reserve_attempt(self.conn, "facebook", EVENT)
    ...
    def test_recovery_cannot_replace_confirmed_id_or_accept_credential_urls(self):
        attempt = reserve_attempt(self.conn, "facebook", EVENT)
        for kwargs in (
            {"remote_id": "token-value"},
            {
                "remote_id": "123",
                "remote_url": "https://example.test/?access_token=secret",
            },
        ...
        for kwargs in (
            {"remote_id": "456"},
            {"remote_url": "https://example.test/456"},
        ):
            with self.assertRaises(click.ClickException):
                resolve_attempt(self.conn, attempt, "published", Mock(), **kwargs)
    ...
    def test_each_mutation_stage_and_actual_content_are_durable_before_post(self):
        ...
        configs = {
            ...
            MASTODON: MASTODON.MastodonConfig(
                "https://example.test", "never-store-token"
            ),
            INSTAGRAM: INSTAGRAM.InstagramConfig(
                "123", "never-store-token", "https://example.test/v26.0"
            ),
        }
        for module, config in configs.items():
            platform = module.__name__.split(".")[0].removeprefix("kulturbytes_")
            for with_image in (False, True) if module is not INSTAGRAM else (True,):
                ...
                def respond(request):
                    ...
                    stage = {
                        ...
                        "/api/v1/statuses": "mastodon_status",
                        "/v26.0/123/photos": "facebook_photo",
                        "/v26.0/123/feed": "facebook_feed",
                        "/v26.0/123/media": "instagram_container",
                        "/v26.0/123/media_publish": "instagram_publish",
                    }[request.url.path]
                    ...
                    if module is FACEBOOK:
                        ...
                        if with_image:
                            ...
                            self.assertEqual(
                                parse_qs(request.content.decode())["message"][0],
                                message,
                            )
                        ...
                        if stage == "mastodon_status":
                            self.assertEqual(
                                parse_qs(request.content.decode())["status"][0], message
                            )
                    else:
                        message = INSTAGRAM.build_instagram_caption(canonical(event))
                        if stage == "instagram_container":
                            self.assertEqual(
                                parse_qs(request.content.decode())["caption"][0],
                                message,
                            )
                    self.assertEqual(
                        row["content_sha256"],
                        content_fingerprint(platform, event, message),
                    )
                    self.assertEqual(
                        row["target_ref"],
                        "https://example.test" if module is MASTODON else "123",
                    )
                    self.assertNotIn("never-store-token", str(row))
                    return httpx.Response(
                        200, json={"id": "123", "url": "https://example.test/123"}
                    )

                with (
                    self.subTest(platform=platform, image=with_image),
                    patch.object(module, "DATABASE_PATH", db),
                    httpx.Client(transport=httpx.MockTransport(respond)) as client,
                    patch("click.confirm", return_value=True),
                ):
                    ...
                    expected = {
                        "facebook": [
                            "facebook_photo" if with_image else "facebook_feed"
                        ],
                        "mastodon": (["mastodon_media"] if with_image else [])
                        + ["mastodon_status"],
                        "instagram": ["instagram_container", "instagram_publish"],
                    }[platform]
                    self.assertEqual(stages, expected)
                    self.assertEqual(
                        list_attempts(conn)[0]["mutation_stage"], expected[-1]
                    )

    def test_uncertain_instagram_final_post_keeps_publish_stage_and_remote_success_is_durable(
        self,
    ):
        ...
        for failure in ("timeout", "reset", "503"):
            ...
            def respond(request):
                ...
                if request.url.path == "/image.jpg":
                    return httpx.Response(200, content=b"\xff\xd8\xffjpeg")
                return httpx.Response(
                    200, json={"id": "123", "status_code": "FINISHED"}
                )

            with (
                patch.object(INSTAGRAM, "DATABASE_PATH", db),
                httpx.Client(transport=httpx.MockTransport(respond)) as client,
                patch("click.confirm", return_value=True),
            ):
                ...
                self.addCleanup(conn.close)
                with self.assertRaises(click.ClickException) as error:
                    INSTAGRAM.publish_event(
                        client, conn, canonical(event), False, config=config
                    )
                row = list_attempts(conn)[0]
                self.assertEqual(
                    (row["state"], row["mutation_stage"]),
                    ("publishing", "instagram_publish"),
                )
    ...
    def test_live_finalization_rollback_keeps_the_previously_committed_remote_success(
        self,
    ):
        self.conn.execute(
            "CREATE TRIGGER reject_final_state BEFORE UPDATE ON publication_attempts WHEN NEW.state='published' BEGIN SELECT RAISE(ABORT, 'final state failure'); END"
        )
        self.conn.commit()
        with self.assertRaises(click.ClickException):
            execute_publication(
        ...
        self.assertEqual(
            (row["state"], row["remote_id"], row["mutation_stage"]),
