...
class EnvironmentTests(IsolatedEnvironmentTestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                with patch.object(
                    ...
                    str(
                        Path(directory)
                        / "lib/python/site-packages/kulturbytes_common/environment.py"
                    ),
                ):
                    with patch.dict(os.environ, {"XDG_CONFIG_HOME": directory}):
                        self.assertEqual(
                            module.get_env_file_path(),
                            Path(directory) / "kulturbytes-social/.env",
                        )
                    with patch.dict(os.environ, {"XDG_CONFIG_HOME": "relative"}):
                        self.assertEqual(
                            module.get_env_file_path(),
                            Path.home() / ".config/kulturbytes-social/.env",
                        )
            finally:
                os.chdir(before)
    ...
    def test_literal_parsing_precedence_and_source(self):
        ...
        with (
            patch.dict(
                os.environ,
                {"META_SYSTEM_USER_ACCESS_TOKEN": "ignored", "FACEBOOK_PAGE_ID": "999"},
            ),
    ...
    def test_empty_dotenv_falls_back_and_empty_environment_suppresses_keyring(self):
        ...
        with (
            patch.dict(
                os.environ, {"META_SYSTEM_USER_ACCESS_TOKEN": TOKEN}, clear=True
            ),
            patch.object(credentials, "get_secret") as keyring,
        ):
            self.assertEqual(
                credentials.resolve_credential_source(
                    credentials.META_SYSTEM_USER
                ).source,
                "environment",
            )
            with patch.dict(os.environ, {"META_SYSTEM_USER_ACCESS_TOKEN": ""}):
                self.assertIsNone(
                    credentials.resolve_credential(credentials.META_SYSTEM_USER)
                )
            keyring.assert_not_called()
    ...
    def test_atomic_update_preserves_other_lines_and_deduplicates(self):
        ...
        for token in [TOKEN, "quotes'\\and${literal}\nnewline"]:
            ...
            self.assertEqual(
                environment.get_dotenv_value("META_SYSTEM_USER_ACCESS_TOKEN"), token
            )
    ...
    def test_failed_writes_leave_original_and_no_temporary_file(self):
        ...
        for operation in ("replace", "fsync"):
            ...
            with (
                patch.object(environment.os, operation, side_effect=OSError(TOKEN)),
    ...
    def test_mastodon_uses_shared_dotenv_precedence(self):
        environment.get_env_file_path().write_text(
            "MASTODON_ACCESS_TOKEN=dotenv-token\n"
        )
        with patch.dict(os.environ, {"MASTODON_ACCESS_TOKEN": TOKEN}):
            self.assertEqual(
                credentials.resolve_credential(credentials.MASTODON), "dotenv-token"
            )
...
class BootstrapTests(IsolatedEnvironmentTestCase):
    def invoke(
        ...
        *,
        ...
        def handle(request):
            ...
            )  # Still not persisted during validation.
            ...
            self.assertEqual(
                ...
                f"Bearer {DERIVED if platform == 'facebook' else TOKEN}",
            )
            if fail:
                return httpx.Response(
                    400, json={"error": {"code": 190, "message": quote(TOKEN, safe="")}}
                )
            return httpx.Response(
                200, json={"id": "123", "name": "Page", "username": "profile"}
            )

        env = {"FACEBOOK_PAGE_ID": "999", "INSTAGRAM_USER_ID": "999"}
        if source in ("dotenv", "environment"):
            env["META_SYSTEM_USER_ACCESS_TOKEN"] = (
                TOKEN if source == "environment" else "ignored-env"
            )
        ...
        with (
            ...
            patch.object(
                credentials,
                "get_secret",
                side_effect=lambda service, name: (
                    TOKEN if source == "keyring" and service.endswith("/meta") else None
                ),
            ),
            ...
            patch.object(module, "init_database") as db,
            patch.object(module, "run_publisher") as workflow,
            patch.object(
                credentials,
                "set_dotenv_value",
                side_effect=environment.EnvironmentFileError("Schreibfehler")
                if write_failure
                else original_set,
            ),
        ):
            result = CliRunner().invoke(
                cli,
                [platform, "--publish" if publish else "--check-auth"],
                input=TOKEN + "\n",
            )
        ...
        for platform in ("facebook", "instagram"):
            for source in ("environment", "keyring", "prompt", "dotenv"):
                with self.subTest(platform=platform, source=source):
                    result, original, _ = self.invoke(
                        platform, source=source, tty=source == "prompt"
                    )
                    self.assertEqual(
                    ...
                    path = environment.get_env_file_path()
                    self.assertEqual(
                        environment.get_dotenv_value("META_SYSTEM_USER_ACCESS_TOKEN"),
                        TOKEN,
                    )
    ...
    def test_noninteractive_missing_never_prompts(self):
        for platform in ("facebook", "instagram"):
            result, original, calls = self.invoke(
                platform, source="missing", publish=True
            )
            self.assertNotEqual(result.exit_code, 0)
    ...
    def test_failed_persistence_stops_publication(self):
        for platform in ("facebook", "instagram"):
            result, original, _ = self.invoke(
                platform, publish=True, write_failure=True
            )
            self.assertNotEqual(result.exit_code, 0)
            self.assertEqual(environment.get_env_file_path().read_bytes(), original)
    ...
    def test_status_reports_source_without_validating_or_writing(self):
        for platform in ("facebook", "instagram"):
            for source, label in [
                ...
                path = environment.get_env_file_path()
                path.write_text(
                    f"META_SYSTEM_USER_ACCESS_TOKEN={TOKEN}\n"
                    if source == "dotenv"
                    else "# unchanged\n"
                )
                ...
                with (
                    ...
                    result = CliRunner().invoke(
                        cli, [platform, "--credentials", "status"]
                    )
    ...
    def test_publication_keeps_validated_credential_after_file_changes(self):
        ...
        for platform in ("facebook", "instagram"):
            with patch.object(
                credentials, "set_dotenv_value", side_effect=concurrent_change
            ):
    ...
    def test_dry_run_never_reads_or_writes_auth_file(self):
        ...
        with patch.object(
            environment,
            "read_env_file",
            side_effect=AssertionError("Unexpected auth read"),
        ):
            with tempfile.TemporaryDirectory() as directory:
                result, _ = PublisherTests().run_cli(
                    FACEBOOK, directory, ["--dry-run"], "1\n"
                )
