...
class MetaTests(IsolatedEnvironmentTestCase):
    def invoke(self, platform, *, env=None, stored=None, publish=False, overrides=None, tty=False):
        ...
        def handle(request):
            ...
            if path in overrides:
                ...
                return override(request) if callable(override) else httpx.Response(*override)
            ...
            if path == '/v26.0/123':
                ...
                return httpx.Response(200, json={'id': '123', 'name': 'Kulturbytes', 'username': 'kulturbytes'})
            if path == '/api/events':
                return httpx.Response(200, json={'data': {'events': [SUMMARY]}})
            if path.startswith('/api/event/'):
                return httpx.Response(200, json={'data': EVENT if platform == 'facebook' else IG_EVENT})
    ...
    def test_shared_env_precedence_and_read_only_checks(self):
        for platform in ('facebook', 'instagram'):
            for tty in (False, True):
                result, calls, lookup = self.invoke(platform, tty=tty,
                    stored={('kulturbytes-social/meta', 'system-user-access-token'): 'ignored'})
                self.assertEqual(result.exit_code, 0, result.output + str(result.exception))
                self.assertTrue(all(r.method == 'GET' and r.url.host == 'graph.facebook.com' for r in calls))
    ...
    def test_shared_keyring_precedes_legacy_environment(self):
        for platform in ('facebook', 'instagram'):
            result, _, lookup = self.invoke(platform, env={k: v for k, v in ENV.items() if k != 'META_SYSTEM_USER_ACCESS_TOKEN'},
                stored={('kulturbytes-social/meta', 'system-user-access-token'): SYSTEM})
            self.assertEqual(result.exit_code, 0, result.output)
            lookup.assert_called_once_with('kulturbytes-social/meta', 'system-user-access-token')

    def test_empty_shared_env_suppresses_keyring_and_allows_legacy(self):
        with patch.dict(os.environ, {**ENV, 'META_SYSTEM_USER_ACCESS_TOKEN': ''}, clear=True), \
    ...
    def test_incompatible_instagram_login_fails_before_requests(self):
        result, calls, _ = self.invoke('instagram', env={**ENV, 'INSTAGRAM_LOGIN_TYPE': 'instagram'})
    ...
    def test_full_publish_sequences_and_token_reuse(self):
        for platform in ('facebook', 'instagram'):
            ...
            self.assertEqual(calls[1 if platform == 'facebook' else 0].url.path, '/v26.0/123')
            self.assertEqual(sum(r.method == 'POST' for r in calls), 1 if platform == 'facebook' else 2)

    def test_auth_errors_never_fall_back_or_touch_events_and_database(self):
        for platform in ('facebook', 'instagram'):
            for payload in [{'error': {'code': 190, 'message': SYSTEM}}, {'error': {'code': 200}},
                            {'id': '999', 'name': 'other', 'username': 'other'}, {'id': '123'}, []]:
                with self.subTest(platform=platform, payload=payload):
                    result, calls, _ = self.invoke(platform, publish=True, overrides={
    ...
    def test_shared_status_and_default_management(self):
        for platform in ('facebook', 'instagram'):
            with patch.dict(os.environ, {}, clear=True), \
                ...
                lookup.assert_called_once_with('kulturbytes-social/meta', 'system-user-access-token')
                result = CliRunner().invoke(cli, [platform, '--credentials', 'set'], input=SYSTEM+'\n')
                self.assertEqual(result.exit_code, 0, result.output)
                self.assertNotIn(SYSTEM, result.output)
                save.assert_called_once_with('kulturbytes-social/meta', 'system-user-access-token', SYSTEM)
                CliRunner().invoke(cli, [platform, '--credentials', 'delete'], input='n\n')
                delete.assert_not_called()
                CliRunner().invoke(cli, [platform, '--credentials', 'delete'], input='y\n')
                delete.assert_called_once_with('kulturbytes-social/meta', 'system-user-access-token')

    def test_system_lookup_failure_does_not_attempt_legacy_recovery(self):
        bodies = [{'data': []}, {}, {'data': 'invalid'}, {'data': [{'id': '123', 'name': 'Page'}]},
                  {'error': {'code': 190, 'error_subcode': 463, 'message': SYSTEM}},
                  {'data': [], 'paging': {'next': 'https://evil.example/', 'cursors': {'after': 'x'}}}]
        for body in bodies:
            result, calls, _ = self.invoke('facebook', publish=True, tty=True, overrides={
    ...
    def test_secret_redaction_on_validation_and_transport_failure(self):
        for platform in ('facebook', 'instagram'):
            for name in (SYSTEM, PAGE_TOKEN, quote(SYSTEM, safe='')):
                result, _, _ = self.invoke(platform, overrides={
                    '/v26.0/123': lambda r: httpx.Response(200, json={'id': '123', 'name': name,
            ...
            result, calls, _ = self.invoke(platform, publish=True, overrides={'/v26.0/123': fail})
            self.assertNotEqual(result.exit_code, 0)
