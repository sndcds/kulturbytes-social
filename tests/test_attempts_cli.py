...
from test_publishers import EVENT, FACEBOOK
...
class AttemptsCLITests(IsolatedEnvironmentTestCase):
    def setUp(self):
        ...
        for index, state in enumerate(
            ("reserved", "failed", "publishing", "published", "remote_succeeded")
        ):
            ...
            if state == "failed":
                ...
                self.conn.execute(
                    "UPDATE publication_attempts SET state='publishing' WHERE attempt_uuid=?",
                    (attempt,),
                )
                self.conn.commit()
        ...
        self.conn.execute(
            "UPDATE publication_attempts SET created_at='2020-01-01 00:00:00'"
        )
        self.conn.commit()
    ...
    def invoke(self, *args, expected=0):
        ...
        with (
            ...
            patch(
                "kulturbytes_common.credentials.resolve_credential",
                side_effect=AssertionError("No credentials allowed"),
            ),
            patch(
                "kulturbytes_common.credentials.get_secret",
                side_effect=AssertionError("No keyring allowed"),
            ),
            patch.object(
                FACEBOOK,
                "load_config",
                side_effect=AssertionError("No auth config allowed"),
            ),
        ):
            result = CliRunner().invoke(
                cli, ["attempts", "list", "--platform", "facebook", *args]
            )
        self.assertEqual(
            result.exit_code, expected, result.output + str(result.exception)
        )
    ...
    def test_active_state_and_date_filters(self):
        ...
        self.assertNotIn(self.attempts[2], output)
        self.assertIn(
            "Keine Veröffentlichungsversuche", self.invoke("--date-uuid", "unknown")
        )
        self.assertIn(
            self.attempts[4], self.invoke("--active", "--state", "remote_succeeded")
        )
