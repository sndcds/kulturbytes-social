...
from test_publishers import ENV as PUBLISHER_ENV
...
class DateIdentityTests(IsolatedEnvironmentTestCase):
    ...
    def run_cli(self, module, database, details, args, user_input):
        ...
        def respond(request):
            ...
            if request.url.path == "/api/events":
                return httpx.Response(
                    200, json={"data": {"events": [SUMMARY] * len(details)}}
                )
        ...
        with (
            ...
            patch.dict(
                os.environ,
                {**PUBLISHER_ENV, **ENV, "META_SYSTEM_USER_ACCESS_TOKEN": ""},
                clear=True,
            ),
    ...
    def test_invalid_detail_identity_blocks_all_publishers(self):
        invalid_dates = [
            dict(EVENT["date"], uuid="date-2"),
            {k: v for k, v in EVENT["date"].items() if k != "uuid"},
            dict(EVENT["date"], uuid=None),
            dict(EVENT["date"], uuid=""),
            None,
            "invalid",
            {},
        ]
                        ...
                        for existing in [False, True]:
                            with (
                                ...
                                result, requests, publish = self.run_cli(
                                    module, database, [event], args, user_input
                                )
                                self.assertEqual(
                                    result.exit_code, 1 if direct else 0, result.output
                                )
    ...
    def test_parent_and_revision_mismatches_block_remote_mutation(self):
        for module in PLATFORMS:
            for field, check in (("parent", 1), ("revision", 3)):
                with (
                    self.subTest(platform=module.__name__, field=field),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    event = deepcopy(EVENT)
                    if field == "parent":
    ...
    def test_matching_identity_reaches_platform_in_direct_mode(self):
        for module in PLATFORMS:
            for dry_run in [False, True]:
                with (
                    self.subTest(platform=module.__name__, dry_run=dry_run),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    ...
                    publish.assert_called_once()
                    self.assertEqual(
                        publish.call_args.args[2].id, EVENT["date"]["uuid"]
                    )
                    self.assertEqual(
                        publish.call_args.args[2].date, EVENT["date"]["start_date"]
                    )
                    self.assertEqual(
                        publish.call_args.args[2].location, EVENT["date"]["venue_name"]
                    )
