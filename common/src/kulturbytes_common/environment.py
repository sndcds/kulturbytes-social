...
def read_env_file() -> dict[str, str | None]:
    ...
    return {
        item.key: item.value
        for item in _bindings(_read_text(get_env_file_path()))
...
def _inline_comment(binding: Binding) -> str:
    ...
    tail = (
        raw.split("=", 1)[1].lstrip() if "=" in raw else raw[len(binding.key or "") :]
    )
    if tail.startswith(("'", '"')):
        quote = tail[0]
        index = 1
...
def set_dotenv_value(name: str, value: str) -> None:
    ...
    try:
        ...
        for item in bindings:
            ...
            if not replaced:
                output.append(
                    f"{name}={quoted}" + (" " + comment if comment else "") + newline
                )
                replaced = True
            elif comment:
        ...
        descriptor, temporary = tempfile.mkstemp(
            prefix=".env.", suffix=".tmp", dir=path.parent
        )
        ...
        raise EnvironmentFileError(
            ".env konnte nicht sicher gespeichert werden; Veröffentlichung abgebrochen."
        ) from None
    finally:
...
def secure_env_file() -> None:
    ...
    try:
        ...
        if not stat.S_ISREG(info.st_mode) or info.st_uid != os.getuid():
            raise EnvironmentFileError(
                ".env kann nicht sicher mit restriktiven Rechten verwaltet werden."
            )
        path.chmod(0o600)
    except OSError:
        raise EnvironmentFileError(
            ".env-Dateirechte konnten nicht gesichert werden."
        ) from None
