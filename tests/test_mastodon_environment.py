...
class MastodonEnvironmentTests(IsolatedEnvironmentTestCase):
    def test_all_credentials_share_source_and_empty_value_semantics(self):
        ...
        for credential in (
            ...
            credentials.FACEBOOK_PAGE,
            credentials.FACEBOOK_USER,
            ...
            for dotenv, env, stored, expected, source in cases:
                with self.subTest(credential=credential.env_name, source=source):
                    ...
                    with (
                        patch.dict(
                            os.environ,
                            {credential.env_name: env} if env is not None else {},
                            clear=True,
                        ),
                        patch.object(
                            credentials, "get_secret", return_value=stored
                        ) as keyring,
                    ):
                        resolved = credentials.resolve_credential_source(credential)
                    self.assertEqual(
                        (resolved.value, resolved.source), (expected, source)
                    )
                    if source == "keyring":
    ...
    def test_base_url_precedence_and_normalization(self):
        for dotenv, env, expected in [
            (BASE + "/", "https://ignored.test", BASE),
            (None, BASE, BASE),
            (None, None, "https://norden.social"),
        ]:
            environment.get_env_file_path().write_text(
                f"MASTODON_BASE_URL={dotenv}\n" if dotenv else ""
            )
            with patch.dict(
                os.environ, {"MASTODON_BASE_URL": env} if env else {}, clear=True
            ):
    ...
    def test_dotenv_base_url_retains_validation(self):
        for invalid in (
            ...
            environment.get_env_file_path().write_text(
                f"MASTODON_BASE_URL='{invalid}'\n"
            )
            with (
                patch.dict(os.environ, {"MASTODON_BASE_URL": BASE}),
    ...
    def test_auth_and_status_with_only_dotenv_are_read_only(self):
        ...
        for failed in (False, True):
            ...
            def respond(request):
                ...
                self.assertEqual(
                    str(request.url), BASE + "/api/v1/accounts/verify_credentials"
                )
                self.assertEqual(request.headers["Authorization"], f"Bearer {TOKEN}")
                return (
                    httpx.Response(401, json={"error": TOKEN})
                    if failed
                    else httpx.Response(200, json={"id": "1", "acct": "kulturbytes"})
                )
            ...
            with (
                ...
                status = CliRunner().invoke(
                    cli, ["mastodon", "--credentials", "status"]
                )
    ...
    def run_publisher(self, *, publish, image=False):
        ...
        def respond(request):
            ...
            if request.url.path == "/api/v2/instance":
                self.assertNotIn("Authorization", request.headers)
                return httpx.Response(
                    200, json={"configuration": {"statuses": {"max_characters": 750}}}
                )
            ...
            return httpx.Response(
                200, json={"id": "status-1", "url": BASE + "/@kulturbytes/status-1"}
            )
    ...
    def test_publish_uses_one_dotenv_configuration_for_status_and_media(self):
        for image in (False, True):
            ...
            self.assertEqual(
                sum(r.url.path == "/api/v2/media" for r in calls), int(image)
            )
            self.assertEqual(
                sum(r.url.path == "/api/v1/media/media-1" for r in calls), int(image)
            )
