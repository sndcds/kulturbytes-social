...
CASES = [
    (FACEBOOK, secrets.FACEBOOK_PAGE, "page"),
    (FACEBOOK, secrets.FACEBOOK_USER, "user"),
    (instagram, secrets.INSTAGRAM, "access"),
    (MASTODON, secrets.MASTODON, "access"),
    (FACEBOOK, secrets.META_SYSTEM_USER, "meta"),
    (instagram, secrets.META_SYSTEM_USER, "meta"),
]
...
class CredentialTests(IsolatedEnvironmentTestCase):
    ...
    def test_env_override_including_empty_never_uses_keyring(self):
        ...
        for _, credential, _ in CASES:
            for value, expected in [(TOKEN, TOKEN), ("", None), ("  ", None)]:
                with (
                    self.subTest(credential=credential, value=value),
                    patch.dict(os.environ, {credential.env_name: value}),
                ):
        ...
        for _, credential, _ in CASES:
            for value in [TOKEN, None]:
                with self.subTest(credential=credential, value=value):
                    ...
                    self.get_password.assert_called_with(
    ...
    def test_backend_failures_are_sanitized(self):
        for failing in [
            ...
            with self.subTest(operation=failing):
                ...
                operation = (
                    (lambda: secrets.set_secret("service", "user", TOKEN))
                    if failing is self.set_password
                    else (
                        (lambda: secrets.delete_secret("service", "user"))
                        if failing is self.delete_password
                        else (lambda: secrets.get_secret("service", "user"))
                    )
                )
    ...
    def test_status_presence_without_secret_details(self):
        for module, credential, selected in CASES:
            for present in [False, True]:
                with self.subTest(
                    platform=module.__name__, selected=selected, present=present
                ):
                    self.get_password.return_value = TOKEN if present else None
                    result = self.invoke(
                        module, ["--credentials", "status", "--credential", selected]
                    )
                    self.assertIn(
                        credential.label
                        + (" vorhanden" if present else " nicht vorhanden"),
                        result.output,
                    )
    ...
    def test_set_uses_hidden_prompt_and_stable_names(self):
        ...
        for module, credential, selected in CASES:
            with (
                self.subTest(platform=module.__name__, selected=selected),
                patch.object(
                    click,
                    "prompt",
                ...
                self.invoke(
                    module,
                    ["--credentials", "set", "--credential", selected],
                    TOKEN + "\n",
                )
                self.set_password.assert_called_once_with(
                    credential.service, credential.username, TOKEN
                )
                self.assertTrue(prompt.call_args.kwargs["hide_input"])
        self.set_password.reset_mock()
        self.invoke(FACEBOOK, ["--credentials", "set"], TOKEN + "\n")
        self.set_password.assert_called_once_with(
            secrets.META_SYSTEM_USER.service, "system-user-access-token", TOKEN
        )
    ...
    def test_delete_confirmation_missing_and_errors(self):
        for module, credential, selected in CASES:
            ...
            with self.subTest(platform=module.__name__, selected=selected):
                ...
                self.delete_password.assert_called_once_with(
                    credential.service, credential.username
                )
    ...
    def test_management_conflicts_and_no_token_argument(self):
        for module in [FACEBOOK, MASTODON, instagram]:
            for args in [
                ["--credentials", "set", "--publish"],
                ["--credentials", "delete", "--check-auth"],
                ["--credential", "page" if module is FACEBOOK else "access"],
                ["--token", TOKEN],
            ]:
        ...
        for module, credential, _ in [CASES[0], CASES[2], CASES[3]]:
            for env_token in [False, True]:
                for failed in [False, True]:
                    with self.subTest(
                        platform=module.__name__, env_token=env_token, failed=failed
                    ):
                        self.backend.reset_mock()
                        self.backend.side_effect = (
                            RuntimeError(TOKEN) if env_token else None
                        )
                        self.get_password.return_value = TOKEN
                        env = {
