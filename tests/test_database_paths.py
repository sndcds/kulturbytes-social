...
class DatabasePathTests(IsolatedEnvironmentTestCase):
    def test_platform_precedence(self):
        for platform in database.PLATFORM_COLUMNS:
            ...
            with patch.dict(os.environ, {name: 'env.sqlite3', 'DATABASE_PATH': '/tmp/legacy.sqlite3'}, clear=True):
                environment.get_env_file_path().write_text(f'{name}=local.sqlite3\n')
                self.assertEqual(database.get_database_path(platform).name, 'local.sqlite3')
                environment.get_env_file_path().write_text('')
                self.assertEqual(database.get_database_path(platform).name, 'env.sqlite3')
                with patch.dict(os.environ, {name: '/tmp/specific.sqlite3'}):
                    self.assertEqual(database.get_database_path(platform), Path('/tmp/specific.sqlite3'))
    ...
    def test_same_file_cannot_accept_different_platform_schemas(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for module in (MASTODON, instagram):
                with patch.object(module, 'DATABASE_PATH', path), self.assertRaises(click.ClickException):
                    module.init_database()
            with sqlite3.connect(path) as conn:
                self.assertEqual(conn.execute('SELECT platform FROM publisher_metadata').fetchone(), ('facebook',))

    def test_legacy_schema_is_checked_before_metadata_is_written(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory)/'legacy.sqlite3'
            with sqlite3.connect(path) as conn:
                conn.execute('CREATE TABLE published_events (date_uuid TEXT PRIMARY KEY, facebook_post_id TEXT)')
