...
class TimezoneTests(IsolatedEnvironmentTestCase):
    def test_summer_and_winter_midnight(self):
        for instant, expected in [
            ("2026-07-01T22:30:00+00:00", "2026-07-02"),
            ...
            for delta in (-1, 0, 1):
                item = ContentItem(
                    title="Calendar", date=(today + timedelta(days=delta)).isoformat()
                )
                ...
                publish = Mock()
                with patch(
                    "kulturbytes_common.workflow.application_today", return_value=today
                ):
                    run_publisher(
