...
import click
import httpx
from canonical_support import build_hashtags, canonical, get_event_url
from click.testing import CliRunner
from dotenv_support import IsolatedEnvironmentTestCase
from test_publishers import ENV, EVENT, SUMMARY
from test_publishers import MASTODON as mastodon
...
class MastodonLimitTests(IsolatedEnvironmentTestCase):
    def test_instance_limit_and_fallback(self):
        payloads = [
            ({"configuration": {"statuses": {"max_characters": 750}}}, 750),
            ({"configuration": {"statuses": {"max_characters": 500}}}, 500),
        ]
        for value in [None, 0, -1, True, "750", 750.5, [], {}]:
            payloads.append(
                ({"configuration": {"statuses": {"max_characters": value}}}, 500)
            )
        payloads += [
            (value, 500)
            for value in [
        ...
        responses = [
            (httpx.Response(200, json=payload), expected)
            for payload, expected in payloads
        ]
        responses += [
            (httpx.Response(500), 500),
            (httpx.Response(200, text="invalid"), 500),
            (httpx.ReadTimeout("timeout"), 500),
            (httpx.Response(401), 500),
        ]
        for response, expected in responses:
            with self.subTest(response=response, expected=expected):
                ...
                def respond(request):
                    ...
                    self.assertEqual(
                        str(request.url), "https://example.test/api/v2/instance"
                    )
                ...
                with (
                    ...
                    self.assertEqual(
                        mastodon.get_status_limit(client, "https://example.test"),
                        expected,
                    )
                self.assertEqual(
                    len(requests), 3 if isinstance(response, httpx.ReadTimeout) else 1
                )

    def test_short_message_unchanged(self):
        self.assertEqual(
            mastodon.build_mastodon_message(canonical(EVENT)),
    ...
    def test_optional_metadata_dropped_whole_in_priority_order(self):
        ...
        message = mastodon.build_mastodon_message(
            canonical(event), required_length + len("Untertitel") + 1
        )
    ...
    def run_cli(self, directory, details, limit, args, user_input):
        ...
        summaries = [
            dict(
                SUMMARY,
                date_uuid=e["date"]["uuid"],
                date_slug=e["date"]["slug"],
        ...
        def respond(request):
            ...
            if path == "/api/v2/instance":
                ...
                return httpx.Response(
                    200, json={"configuration": {"statuses": {"max_characters": limit}}}
                )
    ...
    def test_once_per_run_and_preview_matches_actual_status(self):
        ...
        for limit in [500, 750, None]:
            with self.subTest(limit=limit), tempfile.TemporaryDirectory() as directory:
                with patch.object(
                    mastodon, "render_post", wraps=mastodon.render_post
                ) as build:
                    result, requests = self.run_cli(
                        directory, events, limit, ["--publish"], "all\ny\ny\n"
                    )
                self.assertEqual(result.exit_code, 0, result.output)
                self.assertEqual(build.call_count, 2)
                self.assertEqual(
                    sum(r.url.path == "/api/v2/instance" for r in requests), 1
                )
                posted = [
                    parse_qs(r.content.decode())["status"][0]
                    for r in requests
                    if r.method == "POST"
                ]
    ...
    def test_exact_required_boundary_and_oversized_hashtags(self):
        ...
        self.assertEqual(
            mastodon.build_mastodon_message(canonical(event), len(fixed)), fixed
        )
        ...
        for dry_run in [False, True]:
            with (
                ...
                with patch.object(mastodon, "DATABASE_PATH", database):
                    ...
                    mastodon.remember_post(
                        conn, EVENT, "old-post", "https://example.test/old"
                    )
