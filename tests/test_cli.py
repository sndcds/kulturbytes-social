...
class UnifiedCliTests(IsolatedEnvironmentTestCase):
    def test_root_help_and_unknown_command(self):
        ...
        for platform in ['facebook', 'mastodon', 'instagram']:
            ...
            for option in ['--dry-run', '--publish', '--check-auth', '--credentials',
                           '--event-uuid', '--date-identifier', '--city', '--limit', '--include-published']:
    ...
    def test_only_root_executable_and_no_wrappers(self):
        ...
        self.assertEqual(root['project']['scripts'], {'kulturbytes-social': 'kulturbytes_social.cli:cli'})
        for platform in ['facebook', 'mastodon', 'instagram']:
    ...
    def test_installed_entry_point_outside_checkout(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            for args in [[], ['facebook'], ['mastodon'], ['instagram']]:
                result = subprocess.run([str(executable), *args, '--help'], cwd=directory,
    ...
    def test_checkout_database_paths_are_cwd_independent(self):
        with patch.dict(os.environ, {}, clear=True):
            before = {platform: database.get_database_path(platform) for platform in ['facebook', 'mastodon', 'instagram']}
            with tempfile.TemporaryDirectory() as directory, chdir(directory):
                for platform, path in before.items():
                    self.assertEqual(database.get_database_path(platform), path)
                    self.assertEqual(path, ROOT / platform / f'{platform}_posts.sqlite3')
            with patch.dict(os.environ, {'DATABASE_PATH': 'custom.sqlite3'}):
                self.assertEqual(database.get_database_path('facebook'), ROOT / 'facebook/custom.sqlite3')
            with patch.dict(os.environ, {'DATABASE_PATH': '/absolute/custom.sqlite3'}):
                self.assertEqual(database.get_database_path('facebook'), Path('/absolute/custom.sqlite3'))

    def test_installed_database_defaults_use_user_data(self):
        with tempfile.TemporaryDirectory() as directory:
            home = Path(directory)
            source = home / 'venv/lib/python3.12/site-packages/kulturbytes_common/database.py'
            with patch.object(database, '__file__', str(source)), patch.object(Path, 'home', return_value=home):
                with patch.dict(os.environ, {}, clear=True):
                    expected = home / '.local/share/kulturbytes-social/mastodon_posts.sqlite3'
                    self.assertEqual(database.get_database_path('mastodon'), expected)
                with patch.dict(os.environ, {'XDG_DATA_HOME': str(home / 'data')}, clear=True):
                    self.assertEqual(database.get_database_path('instagram'), home / 'data/kulturbytes-social/instagram_posts.sqlite3')
