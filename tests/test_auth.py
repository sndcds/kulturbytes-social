...
from test_instagram import EVENT as INSTAGRAM_EVENT
...
class AuthTests(IsolatedEnvironmentTestCase):
    def setUp(self):
        self.secret_lookup = patch(
            "kulturbytes_common.credentials.get_secret", return_value=None
        )
    ...
    def run_auth(self, module, response, *, env=None, args=None, expected_exit=0):
        ...
        with (
            ...
            patch(
                "kulturbytes_common.auth.httpx.Client",
                side_effect=make_client,
            ),
            patch.object(module, "init_database") as database,
            patch.object(
                module,
                "run_publisher",
            ) as workflow,
            patch.object(module, "publish_event") as publish,
        ):
            result = CliRunner().invoke(
                cli,
                [module.__name__.split(".")[0].removeprefix("kulturbytes_")]
                + (args or ["--check-auth"]),
            )
        self.assertEqual(
        ...
        self.assertEqual(
            len(requests),
            3
            if isinstance(response, (httpx.ConnectError, httpx.ReadTimeout))
            or isinstance(response, httpx.Response)
            ...
            for login in ["instagram", "facebook"]:
                with self.subTest(platform=module.__name__, login=login):
                    ...
                    result, request = self.run_auth(
                        ...
                        env=dict(ENV, INSTAGRAM_LOGIN_TYPE=login),
                        args=[
                            "--check-auth",
                            "--publish",
                            "--city",
                    ...
                    if module is MASTODON:
                        self.assertEqual(
                            str(request.url),
                            "https://norden.social/api/v1/accounts/verify_credentials",
                        )
                        self.assertIn("@kulturbytes@norden.social", result.output)
                    else:
                        host = (
                            "graph.instagram.com"
                            if module is instagram and login == "instagram"
                        ...
                        self.assertEqual(
                            ...
                            "id,name" if module is FACEBOOK else "id,username",
                        )
    ...
    def test_auth_failures_and_redaction(self):
        for module in PLATFORMS:
            for status in [400, 401, 403, 500]:
                for payload in [
                    {"error": {"code": 190, "message": TOKEN}},
                    {
                        "error": {
                            "code": 190,
                            "error_subcode": 463,
                            "message": quote(TOKEN, safe=""),
                        }
                    },
                    {"error": TOKEN},
                ]:
                    with self.subTest(
                        platform=module.__name__, status=status, payload=payload
                    ):
                        result, _ = self.run_auth(
                            module,
                        ...
                        if (
                            ...
                            and payload["error"].get("error_subcode") == 463
                        ):
                            self.assertIn("abgelaufen", result.output)
        for module in PLATFORMS:
            with self.subTest(platform=module.__name__):
                self.run_auth(
                    module, httpx.Response(200, json={"error": TOKEN}), expected_exit=1
                )
                ...
                self.run_auth(
                    module,
                    httpx.Response(
                        302, headers={"Location": "https://example.test/" + TOKEN}
                    ),
                    expected_exit=1,
                )
        ...
        for module in PLATFORMS:
            for payload in [{}, [], None, {"id": "123"}]:
                with self.subTest(platform=module.__name__, payload=payload):
                    self.run_auth(
                        module, httpx.Response(200, json=payload), expected_exit=1
                    )
        ...
        for module in PLATFORMS:
            with (
                ...
                result = subprocess.run(
                    [sys.executable, "-c", f"import {module.__name__}"],
                    capture_output=True,
                    text=True,
                    check=False,
                )
    ...
    def test_missing_credentials_fail_before_network_or_database(self):
        for module in PLATFORMS:
            for option in ["--publish", "--check-auth"]:
                with (
                    self.subTest(platform=module.__name__, option=option),
                    patch.dict(os.environ, {}, clear=True),
                    patch(
                        "httpx.Client",
                    ) as client,
    ...
    def test_strict_config_and_secret_repr(self):
        ...
        for module, variable, values in cases:
            for value in values:
                with (
                    self.subTest(
                        platform=module.__name__, variable=variable, value=value
                    ),
                    patch.dict(
                        os.environ,
                        dict(ENV, **{variable: value}),
                        clear=True,
                    ),
    ...
    def test_publication_requests_use_lazy_credentials(self):
        cases = [
            ...
            (
                FACEBOOK,
                FACEBOOK.publish_facebook_photo,
                "/v26.0/123/photos",
                "post-123",
            ),
        ...
        for module, publish, path, expected in cases:
            for status in [200, 401]:
                with (
                    self.subTest(function=publish.__name__, status=status),
                    ...
                    with (
                        ...
                        patch.object(
                            module,
                            "download_image",
                            return_value=(b"image", "image/jpeg", "image.jpg"),
                        ),
                    ):
                        if status == 200:
                            self.assertEqual(
                                publish(client, canonical(EVENT)), expected
                            )
                        else:
    ...
    def test_mastodon_media_poll_uses_lazy_credentials(self):
        with patch.dict(os.environ, ENV, clear=True):
            ...
            def respond(request):
                ...
                self.assertEqual(request.headers["Authorization"], f"Bearer {TOKEN}")
                return httpx.Response(
                    200, json={"url": "https://example.test/image.jpg"}
                )
        ...
        for module in PLATFORMS:
            with (
                ...
                with (
                    ...
                    patch.dict(os.environ, {}, clear=True),
                    patch.object(module, "DATABASE_PATH", database),
                    patch(
                        "kulturbytes_common.workflow.httpx.Client",
                        return_value=client,
                    ),
                ):
                    result = CliRunner().invoke(
