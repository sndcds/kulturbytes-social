...
import click
import httpx
from canonical_support import canonical, get_event_url
...
SUMMARY = {
    ...
    "title": "Kulturabend",
    "venue_city": "Flensburg",
    "summary": "Zusammenfassung aus der Liste.",
}
ENV = {
...
class InstagramTests(IsolatedEnvironmentTestCase):
        ...
        def respond(request):
            ...
            if path == "/v26.0/123":
                ...
                return httpx.Response(
                    200, json={"id": "123", "username": "kulturbytes"}
                )
            ...
            if path == "/v26.0/123/media_publish":
                self.assertEqual(request.method, "POST")
                self.assertEqual(
                    parse_qs(request.content.decode()), {"creation_id": ["456"]}
                )
    ...
    def test_direct_dry_run_by_both_identifiers(self):
        for identifier in ["date-1", "209901011830"]:
            with self.subTest(identifier=identifier):
                result = self.run_cli(
                    ["--event-uuid", "event-1", "--date-identifier", identifier], env={}
                )
                self.assertIn("DRY RUN", result.output)
    ...
    def test_facebook_login_uses_facebook_host(self):
        ...
        self.assertTrue(
            all(
                r.url.host == "graph.facebook.com"
                for r in self.requests
                if "/v26.0/" in r.url.path
            )
        )
    ...
    def test_api_failures_do_not_record_success_and_redact_token(self):
        for index, path in enumerate(
            ...
            with self.subTest(path=path):
                self.requests.clear()
                self.overrides = {
                    path: lambda r: httpx.Response(
                        400, json={"error": {"message": "secret-token denied"}}
                    )
                }
                result = self.run_cli(DIRECT + ["--publish"], "y\n", expected_exit=1)
    ...
    def test_invalid_creation_response_does_not_publish(self):
        for index, payload in enumerate(
            [{}, {"id": None}, [], {"error": "secret-token"}]
        ):
            self.database = Path(self.directory.name) / f"invalid-response-{index}.db"
            with self.subTest(payload=payload):
                self.overrides = {
                    "/v26.0/123/media": lambda r: httpx.Response(200, json=payload)
                }
                result = self.run_cli(DIRECT + ["--publish"], "y\n", expected_exit=1)
                self.assertNotIn("secret-token", result.output)
                self.assert_unpublished()
    ...
    def test_container_error_and_timeout_do_not_publish(self):
        for index, status in enumerate(
            ["ERROR", "EXPIRED", "PUBLISHED", "IN_PROGRESS", None]
        ):
            self.database = Path(self.directory.name) / f"container-status-{index}.db"
            with self.subTest(status=status):
                self.overrides = {
                    "/v26.0/456": lambda r: httpx.Response(
                        200, json={"status_code": status}
                    )
                }
    ...
    def test_waits_for_processing_before_publishing(self):
        ...
        self.overrides = {
            "/v26.0/456": lambda r: httpx.Response(
                200, json={"status_code": next(statuses)}
            )
        }
    ...
    def test_shared_markdown_normalization(self):
        self.assertEqual(
            strip_markdown(r"**Text** 7\. September [Website](https://example.org)"),
